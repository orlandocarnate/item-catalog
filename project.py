...
from flask import (Flask, render_template, request,
                   redirect, url_for, flash, app, jsonify)
...
CLIENT_ID = (json.loads(open('client_secrets.json', 'r').read())['web']
             ['client_id'])
FB_ID = json.loads(open('fb_client_secrets.json', 'r').read())['web']['app_id']
...
def showLogin():
    ...
    # return "API key is " + apikey + "The current session
    # state is %s" % login_session['state']
...
def categoryPage(category_name):
    ...
    if 'username' not in login_session:
        return render_template('shop/publiccategory.html',
                               category=category,
                               items=items,
                               categories=categories)
    else:
...
def editCategoryPage(category_name):
    ...
    if editCategory.user_id != login_session['user_id']:
        flash('Sorry. You are not authorized to edit this category.')
    ...
    if request.method == 'POST':
        ...
        return render_template('shop/editcategory.html',
...
def newItem(category_name):
    ...
    if request.method == 'POST':
        ...
        user_name = login_session['username']
        return render_template('shop/newitem.html',
...
def editItem(category_name, item_id):
    ...
    if editJewelryItem.user_id != login_session['user_id']:
        flash('Only the creator of this item can edit. '
              'You must create your own Jewelry item to edit.')
